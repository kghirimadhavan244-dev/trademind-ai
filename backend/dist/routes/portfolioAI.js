...
`;
            const reply = await (0, gemini_1.askGemini)(emptyPrompt);
            return res.json({
                success: true,
                analysis: reply,
                summary: {
        ...
        const holdingsSummary = holdingsDetails
            .map((h) => `- ${h.symbol}: ${h.quantity} shares, Avg buy price ₹${h.buyPrice.toFixed(2)} (Total Value: ₹${h.totalValue.toFixed(2)}, Sector: ${h.sector})`)
            .join("\n");
...
`;
        const analysis = await (0, gemini_1.askGemini)(analysisPrompt);
        return res.json({
            success: true,
            analysis,
            summary: {
...
router.get("/brief/:userId", async (req, res) => {
    try {
        ...
        const niftyQuote = await (0, marketService_1.fetchStockQuote)("NIFTY50");
        ...
        const holdingsWithCurrentQuotes = await Promise.all(holdings.map(async (h) => {
            try {
                const q = await (0, marketService_1.fetchStockQuote)(h.symbol);
                return {
                    ...h,
...
`;
        const briefText = await (0, gemini_1.askGemini)(briefPrompt);
        return res.json({
            success: true,
